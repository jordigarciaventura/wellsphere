...
export default function Journal({ initialDate = new Date() }: JournalProps) {
  ...
  const renderCalendar = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const dayEntry = entries[dateKey(dayDate)];
      days.push(
        <Grid item key={i}>
          <Button
          ...
          >
            ...
            {dayEntry && dayEntry.mood && (
              <Typography component="span" sx={{ fontSize: '0.5em', position: 'absolute', bottom: 0, right: 0 }}>
                {dayEntry.mood}
              </Typography>
            )}
          </Button>
  ...
  return (
    <ThemeProvider theme={theme}>
      <Box sx={{ maxWidth: 400, mx: 'auto', p: 2, bgcolor: 'background.default' }}>
        ...
        <Box display="flex" justifyContent="space-between" mb={2}>
          {emojis.map((emoji, index) => (
            <IconButton 
              ...
              sx={{ 
                fontSize: '1.5rem',
                bgcolor: currentEntry.mood === emoji ? 'primary.light' : 'transparent',
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
          <Button onClick={handlePrevMonth} size="small" sx={{ minWidth: 0, p: 0 }}>
            <ChevronLeftIcon />
          </Button>
          <Typography variant="subtitle2">
            {currentDate.toLocaleString('default', { month: 'long' })}
          </Typography>
          <Button onClick={handleNextMonth} size="small" sx={{ minWidth: 0, p: 0 }}>
            <ChevronRightIcon />
          </Button>
        </Box>
