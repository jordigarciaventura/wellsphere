...
export async function getUserById(id: number) {
    return await db.select().from(usersTable).where(eq(usersTable.id, id.toString()));
}

export async function updateUser(userId: number, username: string) {
    await db.update(usersTable).set({ name: username}).where(eq(usersTable.id, userId.toString()));
}
